...
vec3 LeavesShading(vec3 L, vec3 v, vec3 n, vec3 albedo, float material){
    ...
    float transmittance = extinction * sigma_s * 0.05;

    float vis = max(0.0, -ndotl * 0.5 + 0.5);

    float phase = mix(HG(mu, 0.8), HG(mu, 0.2), 0.9);

    return scattering * (transmittance * phase * invPi);
}
